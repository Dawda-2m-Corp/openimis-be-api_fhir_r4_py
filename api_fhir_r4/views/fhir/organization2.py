...
from api_fhir_r4.views.fhir.base import BaseFHIRView, BaseMultiserializerFHIRView
...
class OrganisationViewSet2(BaseFHIRView, MultiIdentifierRetrieverMixin,
                   MultiIdentifierUpdateMixin, viewsets.ModelViewSet):
    retrievers = [UUIDIdentifierModelRetriever, CodeIdentifierModelRetriever]
    lookup_field = 'uuid'
    serializer_class = PolicyHolderOrganisationSerializer
    ...
    def list (self, request, *args, **kwargs): 
        ...
        if identifier:
            return self.retrieve(request, *args, **{**kwargs, 'code':identifier })
        else:
            queryset = queryset.filter()
        serializer =  PolicyHolderOrganisationSerializer(self.paginate_queryset(queryset), many=True) 
